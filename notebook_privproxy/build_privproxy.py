"""Build priv-proxy notebook — measure the REAL multi-message replay slowdown under a
BLOCKING guardrail, via the faithful gateway path (not the bench's 16s artifact).

We monkeypatch build_guardrail_configs so the actual gateway replays the SAME candidate
list against BOTH {public: OptimalGuardrail} and {denyhttp: DenyHttpPost}. DenyHttpPost
denies every http.post (models the private provenance guardrail's worst case). The gateway
prints each guardrail's replay wall-time -> ratio = the true PRIVATE_FACTOR for multi-message,
INCLUDING any 8-hop retry amplification a blocked call triggers. gpt_oss only, small fixed
grid (F=150 x M=2) -> ~10-20 min. Definitive input for sizing the next submission rung.
"""
from __future__ import annotations
import json
from pathlib import Path

OUT = Path(__file__).resolve().parent / "priv-proxy.ipynb"
...
# Fixed-grid mini attack: F findings x M msgs, suppressed template, NO probe (deterministic, fast).
MINI_ATTACK = r'''
import glob, sys
from pathlib import Path
...
nb = {
    "cells": [
        md("# priv-proxy — multi-message replay slowdown under a blocking guardrail (faithful gateway path)"),
        code(SETUP),
        code("MINI_ATTACK = r'''" + MINI_ATTACK + "'''\nprint('mini attack defined', len(MINI_ATTACK), 'chars')\n"),
        code(RUN),
    ],
